...
/**
 * A custom event generator that extends MessageEvent and implements EventQueue.
 * Ensures keyEncryption is available before creating message events.
 */
public class CustomMessageEventGenerator implements EventQueue {
     ...
     */
    public CustomMessageEventGenerator(Settings s) {
    ...
    /**
     * Generates the next event in the queue.
     *
     * @return The next event or an event with time Double.MAX_VALUE if no events are left.
     */
    @Override
    public ExternalEvent nextEvent() {

        // Generate the next message creation event
        int responseSize = 0; // No response message
        int msgSize = drawMessageSize();
        int interval = drawNextEventTimeDiff();
        int from = drawHostAddress(this.hostRange);
        int to = drawToAddress(this.hostRange, from);

        // Create the message event
        MessageCreateEventNAKT mce = new MessageCreateEventNAKT(from, to, this.getID(), msgSize, responseSize, this.nextEventsTime);
        this.nextEventsTime += interval;

        // Stop generating events if the next event is beyond the allowed time range
     ...
     *
     * @return The time of the next event or Double.MAX_VALUE if no events are left.
     */
    @Override
     ...
     *
     * @param hostRange The range of host addresses.
     ...
     */
    private int drawToAddress(int[] hostRange, int from) {
        int to;
        do {
     ...
     *
     * @return A random message size.
     */
    private int drawMessageSize() {
    ...
    protected int drawNextEventTimeDiff() {
        return (msgInterval[0] + msgInterval[1]) / 2;
    }
